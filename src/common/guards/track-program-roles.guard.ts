...
@Injectable()
export class TrackProgramRolesGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Get trackId from route params
    const trackId = request.params.trackId || request.params.id;

    if (!trackId) {
      throw new ForbiddenException('Track ID is required');
    }

    // Resolve track to program
    ...
    request.programId = track.programId;
